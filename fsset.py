    ...
    def setupDevice (self, chroot="/", devPrefix='/tmp'):
        ...
        for volume in self.physicalVolumes:
            ...
            if not self.isSetup:
                ...
                lvm.writeForceConf()
                rc = lvm.lvmExec("pvcreate", "-ff", "-y", "-v", node)
                if rc:
                    raise LvmError("pvcreate", node)
                lvm.unlinkConf()
        ...
        if not self.isSetup:
            ...
            rc = lvm.lvmExec(*args)
            if rc:
                raise LvmError("vgcreate", self.name)
...
class LogicalVolumeDevice(Device):
    ...
    def setupDevice(self, chroot="/", devPrefix='/tmp', vgdevice = None):
        if not self.isSetup:
            lvm.writeForceConf()
            rc = lvm.lvmExec("lvcreate", "-L", "%dM" % (self.size,), "-n",
                             self.name, "-An", self.vgname)
            if rc:
                raise LvmError("lvmcreate", self.name)
            lvm.unlinkConf()
            self.isSetup = 1
