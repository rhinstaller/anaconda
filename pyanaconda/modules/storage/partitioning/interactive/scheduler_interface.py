...
class DeviceTreeSchedulerInterface(DeviceTreeInterface):
    ...
    def GetDeviceTypesForDevice(self, devie_id: Str) -> List[Int]:
        """Collect supported device types for the given device.

        :param devie_id: a device ID
        :return: a list of device types
        """
        return self.implementation.get_device_types_for_device(devie_id)

    def GetFileSystemsForDevice(self, devie_id: Str) -> List[Str]:
        """Get supported file system types for the given device.

        :param devie_id: a device ID
        :return: a list of file system names
        """
        return self.implementation.get_file_systems_for_device(devie_id)
    ...
    def ResetDevice(self, devie_id: Str):
        """Reset the specified device in the storage model.

        FIXME: Merge with DestroyDevice.

        :param devie_id: ID of the device
        :raise: StorageConfigurationError in case of failure
        """
        self.implementation.reset_device(devie_id)

    def DestroyDevice(self, devie_id: Str):
        """Destroy the specified device in the storage model.

        :param devie_id: ID of the device
        :raise: StorageConfigurationError in case of failure
        """
        self.implementation.destroy_device(devie_id)
