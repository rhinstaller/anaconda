...
class BootcSourceModule(PayloadSourceBase):
    ...
    def process_kickstart(self, data):
        ...
        configuration.osname = data.bootcsetup.osname
        configuration.remote = data.bootcsetup.remote
        configuration.url = data.bootcsetup.url
        configuration.ref = data.bootcsetup.ref
        configuration.gpg_verification_enabled = not data.bootcsetup.nogpg
        self.set_configuration(configuration)

    def setup_kickstart(self, data):
        """Setup the kickstart data."""
        data.bootcsetup.osname = self.configuration.osname
        data.bootcsetup.remote = self.configuration.remote
        data.bootcsetup.url = self.configuration.url
        data.bootcsetup.ref = self.configuration.ref
        data.bootcsetup.nogpg = not self.configuration.gpg_verification_enabled
        data.bootcsetup.seen = True
