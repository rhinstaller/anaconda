...
class LocalizationService(KickstartService):
    ...
    def get_keyboard_layouts(self):
        ...
        langtable_keyboards = get_common_keyboard_layouts()
        common_languages_names = [self.get_language_data(lang).english_name for lang in get_common_languages()]

        layouts = []
        for name, info in keyboards:
            if name:
                is_common_lang = len(info.langs) > 0 and info.langs[0].split(";")[0] in common_languages_names
                layout = KeyboardLayout()
                layout.layout_id = name
                layout.description = self.get_layout_variant_description(name, with_lang=True, xlated=True)
                layout.is_common = name in langtable_keyboards and is_common_lang
