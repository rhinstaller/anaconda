...
class TimeSpoke(FirstbootSpokeMixIn, NormalTUISpoke):
    ...
    def apply(self):
        ...
        self.data.timezone.ntpservers = self.ntp_servers.keys()
...
class TimeZoneSpoke(NormalTUISpoke):
    ...
    def input(self, args, key):
        try:
            ...
            elif key.lower() == C_('TUI|Spoke Navigation|Timezone Settings', 'b'):
...
class RemoveNTPServerSpoke(NormalTUISpoke):
    ...
    def apply(self):
        if self._ntp_server_index is not None:
            ntp_server_address = self._time_spoke.ntp_servers.keys()[self._ntp_server_index]
