...
class GRDServer(object):
    ...
    def start_grd_rdp(self):
        # check if RDP username & password are set
        if not self.rdp_password or not self.rdp_username:
            stdoutLog.critical("RDP username or password not set. Aborting.")
            util.ipmi_abort(scripts=self.anaconda.ksdata.scripts)
            sys.exit(1)
        ...
        self.log.info(_("GNOME remote desktop RDP: username and password set"))
        ...
        try:
            ...
            self.log.info(_("GNOME remote desktop RDP hostname: %s"), self.name)
