...
def _run_program(argv, root='/', stdin=None, stdout=None, env_prune=None,
    ...
    """ Run an external program, log the output and return it to the caller
        ...
        :param ignore_decode_errors: whether to ignore decode errors
    ...
    try:
        ...
        if not binary_output:
            output_string = output_string.decode(
                "utf-8",
                errors="strict" if not ignore_decode_errors else "ignore"
            )
...
def execWithCapture(command, argv, stdin=None, root='/',
                    env_prune=None, env_add=None, ignore_decode_errors=False,
