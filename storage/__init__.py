...
""" % time.asctime()
    ...
    def bootDevice(self):
        ...
        if iutil.getPPCMacGen() == "NewWorld":
            ...
            # XXX who cares if PReP is formatted? it has no format.
            partitions = self.devicetree.getDeviceByType("partition")
            for device in partitions:
                if device.partedPartition.getFlag(parted.PARTITION_PREP) and \
                   (bestprep is None or \
                    (bestprep.format.exists and not device.format.exists)):
                    bootDev = device
        elif iutil.isEfi():
        ...
        # XXX should we raise an error if no boot device was found?
        return bootDev
    ...
    def bootloaderChoices(self, bl):
        ...
        elif bootDev.device.getName() == "RAIDDevice":
            ret['boot'] = (bootDev.path, N_("RAID Device"))
            ret['mbr'] = (bl.drivelist[0], N_("Master Boot Record (MBR)"))
        elif iutil.getPPCMacGen() == "NewWorld":
