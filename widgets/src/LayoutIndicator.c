...
//                                                             g_object_ref(self));
    ...
    self->priv->layout = get_current_layout();
...
static void x_state_changed(gint arg2, gboolean arg3, gpointer data) {
...
static void x_config_changed(gpointer data) {
    g_return_if_fail(data);
    AnacondaLayoutIndicator *indicator = ANACONDA_LAYOUT_INDICATOR(data);
