...
class UpdateNvdimmDataTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.parser = kickstart.AnacondaKSParser(self.handler)

    def _read(self, input_ks):
        # pylint: disable=no-value-for-parameter
        self._patched_read(input_ks)

    @patch("blivet.nvdimm.nvdimm")
    def _patched_read(self, input_ks, nvdimm):
        mocked_nvdimm = mock.Mock()
        mocked_nvdimm.namespaces = ["namespace0.0", "namespace1.0"]
        nvdimm.return_value = mocked_nvdimm
