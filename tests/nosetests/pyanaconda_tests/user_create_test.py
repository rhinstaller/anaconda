...
class UserCreateTest(unittest.TestCase):
    ...
    def create_group_test(self):
        """Create a group."""
        self.users.create_group("test_group", root=self.tmpdir)
    ...
    def create_group_gid_test(self):
        ...
        self.users.create_group("test_group", gid=47, root=self.tmpdir)
    ...
    def create_group_exists_test(self):
        ...
        self.assertRaises(ValueError, self.users.create_group, "test_group", root=self.tmpdir)
    ...
    def create_group_gid_exists_test(self):
        ...
        self.assertRaises(ValueError, self.users.create_group, "test_group", gid=47, root=self.tmpdir)

    def create_user_test(self):
        """Create a user."""
        self.users.create_user("test_user", root=self.tmpdir)
    ...
    def create_user_text_options_test(self):
        ...
        self.users.create_user("test_user", gecos="Test User", homedir="/home/users/testuser", shell="/bin/test", root=self.tmpdir)
    ...
    def create_user_groups_test(self):
        ...
        self.users.create_group("test3", root=self.tmpdir)

        # Create a user and add it three groups, two of which do not exist,
        # and one which specifies a GID.
        self.users.create_user("test_user", groups=["test1", "test2(5001)", "test3"], root=self.tmpdir)
    ...
    def create_user_groups_gid_conflict_test(self):
        ...
        # Create one of the groups
        self.users.create_group("test3", gid=5000, root=self.tmpdir)

        # Add test3 to the group list with a different GID.
        self.assertRaises(ValueError, self.users.create_user,
                "test_user", groups=["test3(5002)"], root=self.tmpdir)

    def create_user_password_test(self):
        """Create a user with a password."""

        self.users.create_user("test_user1", password="password", root=self.tmpdir)
        ...
        self.users.create_user("test_user2", password=cryptpw, isCrypted=True, root=self.tmpdir)
        shadow_fields = self._readFields("/etc/shadow", "test_user2")
        self.assertIsNotNone(shadow_fields)
        self.assertEqual(cryptpw, shadow_fields[1])

        # Set an empty password
        self.users.create_user("test_user3", password="", root=self.tmpdir)
    ...
    def create_user_lock_test(self):
        ...
        self.users.create_user("test_user1", lock=True, password="", root=self.tmpdir)
        shadow_fields = self._readFields("/etc/shadow", "test_user1")
        self.assertIsNotNone(shadow_fields)
        self.assertEqual("!", shadow_fields[1])

        # Create a locked password and ensure it can be unlocked (by removing the ! at the front)
        self.users.create_user("test_user2", lock=True, password="password", root=self.tmpdir)
        shadow_fields = self._readFields("/etc/shadow", "test_user2")
        self.assertIsNotNone(shadow_fields)
        self.assertTrue(shadow_fields[1].startswith("!"))
    ...
    def create_user_uid_test(self):
        ...
        self.users.create_user("test_user", uid=1047, root=self.tmpdir)
        pwd_fields = self._readFields("/etc/passwd", "test_user")
        self.assertIsNotNone(pwd_fields)
    ...
    def create_user_gid_test(self):
        ...
        self.users.create_user("test_user", gid=1047, root=self.tmpdir)
    ...
    def create_user_algo_test(self):
        ...
        self.users.create_user("test_user1", password="password", algo="md5", root=self.tmpdir)
        shadow_fields = self._readFields("/etc/shadow", "test_user1")
        self.assertIsNotNone(shadow_fields)
        self.assertTrue(shadow_fields[1].startswith("$1$"))

        self.users.create_user("test_user2", password="password", algo="sha512", root=self.tmpdir)
        shadow_fields = self._readFields("/etc/shadow", "test_user2")
        self.assertIsNotNone(shadow_fields)
    ...
    def create_user_exists_test(self):
        ...
        self.assertRaises(ValueError, self.users.create_user, "test_user", root=self.tmpdir)
    ...
    def create_user_uid_exists_test(self):
        ...
        self.assertRaises(ValueError, self.users.create_user, "test_user", uid=1000, root=self.tmpdir)

    def create_user_gid_exists_test(self):
        """Create a user with a GID of an existing group."""
        self.users.create_group("test_group", gid=5000, root=self.tmpdir)
        self.users.create_user("test_user", gid=5000, root=self.tmpdir)
    ...
    def set_user_ssh_key_test(self):
        ...
        self.users.create_user("test_user", homedir="/home/test_user", root=self.tmpdir)
    ...
    def set_root_password_test(self):
        ...
        self.users.set_root_password(password, root=self.tmpdir)
        shadow_fields = self._readFields("/etc/shadow", "root")
        self.assertEqual(crypt.crypt(password, shadow_fields[1]), shadow_fields[1])

        # Try a different password with isLocked=True
        password = "password2"
        self.users.set_root_password(password, isLocked=True, root=self.tmpdir)
        shadow_fields = self._readFields("/etc/shadow", "root")
        self.assertTrue(shadow_fields[1].startswith("!"))
        self.assertEqual(crypt.crypt(password, shadow_fields[1][1:]), shadow_fields[1][1:])
        ...
        self.users.set_root_password(password, isCrypted=True, root=self.tmpdir)
        shadow_fields = self._readFields("/etc/shadow", "root")
    ...
    def create_user_reuse_home_test(self):
        ...
        self.users.create_user("test_user", homedir="/home/test_user", uid=1000, gid=1000, root=self.tmpdir)
        passwd_fields = self._readFields("/etc/passwd", "test_user")
        self.assertIsNotNone(passwd_fields)
    ...
    def create_user_gid_in_group_list_test(self):
        ...
        self.users.create_user("test_user", gid=1047, groups=["test_group(1047)"], root=self.tmpdir)
